...
public class BalanceSheetStatementRepository {
  ...
  public Set<BalanceSheetStatement> findAllBySymbol(String symbol) {
    List<BalanceSheetStatementEntity> statementEntities = crudRepository.findAllBySymbol(symbol);
    return statementEntities.stream().map(mapper::entityToDto).collect(Collectors.toSet());
  }

  public Set<BalanceSheetStatement> findAllBySymbolAndPeriod(String symbol, String period) {
    List<BalanceSheetStatementEntity> statementEntities = crudRepository.findAllBySymbolAndPeriod(symbol, period);
    return statementEntities.stream().map(mapper::entityToDto).collect(Collectors.toSet());
  }

  public Set<BalanceSheetStatement> saveAll(Statement<Set<BalanceSheetStatement>> statements) {
