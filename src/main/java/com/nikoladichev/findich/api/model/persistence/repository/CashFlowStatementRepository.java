...
public class CashFlowStatementRepository {
  ...
  public Set<CashFlowStatement> findAllBySymbol(String symbol) {
  ...
  public Set<CashFlowStatement> findAllBySymbolAndPeriod(String symbol, String period) {
    List<CashFlowStatementEntity> statementEntities = crudRepository.findAllBySymbolAndPeriod(symbol, period);
    return statementEntities.stream().map(mapper::entityToDto).collect(Collectors.toSet());
  }

  public Set<CashFlowStatement> saveAll(Statement<Set<CashFlowStatement>> statements) {
